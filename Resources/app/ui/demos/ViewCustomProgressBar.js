...
App.ui.demos.createViewCustomProgressBar = {};

(function(){
	
	App.ui.demos.createViewCustomProgressBar = function(){
		var view = App.ui.createViewDemo(demoInfo);

		var pgBar = TitanTricks.ui.createProgressBar({
			min:0,
			max:100,
			left: 20,
			right: 20,
		...
		slideValue.addEventListener('change', function(e){
			pgBar.updateValue(e.source.value);
		});
		view.add(lblValue);
		view.add(slideValue);
	...
	var demoInfo = {
		...
		ios:			true,
		android:		true,
		createView: 	App.ui.demos.createViewCustomProgressBar
	}
